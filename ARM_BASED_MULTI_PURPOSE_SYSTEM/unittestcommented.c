...
if(result1==success)
{
	LOG("CB UNIT TEST:1/7-<bufferfull> PASS\n"); //display pass if success else fail for test 1
	count_pass++;
}
else
{
...
if(result2==success)
{
	LOG("CB UNIT TEST:2/7-<bufferempty> PASS\n");//display pass if success else fail for test 2
	count_pass++;
	}
	else
	{
	LOG("CB UNIT TEST:2/7-<bufferempty> FAIL\n");
	count_fail++;
}
result3=unittest_additemwraparound();
if(result3==success)
{	
    LOG("CB UNIT TEST:3/7-<additemwraparound> PASS\n");//display pass if success else fail for test 3
	count_pass++;
	}
	else
	{
	LOG("CB UNIT TEST:3/7-<additemwraparound> FAIL\n");
	count_fail++;
}
result4=unittest_currentitemsinadd();
if(result4==success)
	{
	LOG("CB UNIT TEST:4/7-<currentitemsinadd> PASS\n");//display pass if success else fail for test 4
	count_pass+;
	}
	else
	{
	LOG("CB UNIT TEST:4/7-<currentitemsinadd> FAIL\n");
	count_fail++;
}
result5=unittest_removeitemwraparound();
if(result5==success)
    {
	LOG("CB UNIT TEST:5/7-<removeitemwraparound> PASS\n");//display pass if success else fail for test 5
	count_pass++;
	}
	else
	{
	LOG("CB UNIT TEST:5/7-<removeitemwraparound> FAIL\n");
	count_fail++;
    }
result6=unittest_currentitemsinremove();
if(result6==success)
	{
	LOG("CB UNIT TEST:6/7-<currentitemsinremove> PASS\n");//display pass if success else fail for test 6
	count_pass++;
    }
	else
    {
	LOG("CB UNIT TEST:6/7-<currentitemsinremove> FAIL\n");
    count_fail++;	
	}
	initialize(&test_buffer);
	if(&test_buffer)
	{
	LOG("CB UNIT TEST:7/7-<initialisation> PASS\n");//display pass if success else fail for test 7
	count_pass++;
	}
	else
	{
	...
	if(count_fail>0)
	{
		LOG("CIRCBUFF UNIT TEST SUITE:FAIL\n")//display unit test suite of pass or fail
	}
	else
